class Package < ApplicationRecord
  ...
  def ping_repo
    ...
    if repo_metadata.blank?
      ...
      connection = Faraday.new 'https://repos.khulnasoft.com' do |builder|
  ...
  def repos_url
    ...
    return if repo_metadata['host'].blank?
    "https://repos.khulnasoft.com/hosts/#{repo_metadata['host']['name']}/repositories/#{repo_metadata['full_name'].gsub('.', '%2E')}"
  end
  ...
  def usage_url
    "https://repos.khulnasoft.com/usage/#{ecosystem}/#{to_param}"
  end

  def dependent_repositories_url
    "https://repos.khulnasoft.com/api/v1/usage/#{ecosystem}/#{to_param}/dependencies"
  end
  ...
  def ping_issues
    ...
    connection = Faraday.new 'https://issues.khulnasoft.com' do |builder|
      builder.use Faraday::FollowRedirects::Middleware
      builder.request :retry, { max: 5, interval: 0.05, interval_randomness: 0.5, backoff_factor: 2 }
      builder.response :json
      builder.request :json
  ...
  def commits_url
    return unless repo_metadata.present?
    "https://commits.khulnasoft.com/hosts/#{repo_metadata['host']['name']}/repositories/#{repo_metadata['full_name']}"
  end

  def commits_api_url
    return unless repo_metadata.present?
    "https://commits.khulnasoft.com/api/v1/hosts/#{repo_metadata['host']['name']}/repositories/#{repo_metadata['full_name']}"
  end
  ...
  def fetch_advisories
    url = "https://advisories.khulnasoft.com/api/v1/advisories?ecosystem=#{ecosystem}&package_name=#{to_param}"
  ...
  def self.update_advisories
    url = "https://advisories.khulnasoft.com/api/v1/advisories?updated_after=#{1.day.ago.iso8601}"
    response = Faraday.get(url)
    return nil unless response.success?
  ...
  def self.update_all_advisories
    url = "https://advisories.khulnasoft.com/api/v1/advisories/packages"
    response = Faraday.get(url)
    return nil unless response.success?
  ...
  def docker_usage_api_url
    "https://docker.khulnasoft.com/api/v1/usage/#{registry.ecosystem_instance.docker_usage_path(self)}"
  end

  def docker_usage_url
    "https://docker.khulnasoft.com/usage/#{registry.ecosystem_instance.docker_usage_path(self)}"
  end
  ...
  def self.update_docker_usages
    url = 'https://docker.khulnasoft.com/api/v1/usage/'
    response = Faraday.get(url)
    return nil unless response.success?
  ...
  def fetch_readme
    ...
    url = 'https://archives.khulnasoft.com/api/v1/archives/readme?url='+download_url
    response = Faraday.get(url)
    return nil unless response.success?
