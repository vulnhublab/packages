class Version < ApplicationRecord
  ...
  def archive_list_url
    "https://archives.khulnasoft.com/api/v1/archives/list?url=#{CGI.escape(download_url)}"
  end

  def archive_contents_url(path)
    "https://archives.khulnasoft.com/api/v1/archives/contents?url=#{CGI.escape(download_url)}&path=#{path}"
  end
  ...
  def digest_url
    "https://digest.khulnasoft.com/digest?url=#{CGI.escape(download_url)}&encoding=hex&algorithm=sha256" # TODO encoding and algorithm should come from ecosystem_instance
  end
  ...
  def diff_url(other_version)
    ...
    "https://diff.khulnasoft.com/diff?url_1=#{url_1}&url_2=#{url_2}"
  end
  ...
  def compare_url
    ...
    "https://diff.khulnasoft.com/diff?url_1=#{download_url}&url_2=#{related_tag['download_url']}"
  end
