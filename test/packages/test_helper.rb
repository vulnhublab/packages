...
class Packages::Test::SingletonClient < Packages::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Packages::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Packages::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @packages = Packages::Test::SingletonClient.instance
  end
